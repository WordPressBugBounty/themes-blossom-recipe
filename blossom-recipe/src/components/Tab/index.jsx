		...
		return (
			<>
				<div className="get-pro">
					<img src={logo} alt={__('Logo', 'blossom-recipe')} />
					<h3>{__('Blossom Recipe Pro', 'blossom-recipe')}</h3>
					<p>{__('Get access to all unlimited features.', 'blossom-recipe')}</p>
					<a className="cw-button-btn primary-btn" target="_blank" href={cw_dashboard.get_pro}>{__('Get Pro', 'blossom-recipe')}</a>
				</div>
			</>
		);
	}
